...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      email,
      password,
      // J'ajoute le username et le rôle 'agent' dans les metadatas
    ...
    if (error) {
      ...
      // Redirection après 2 secondes vers le terminal
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white font-mono flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-black border border-white/10 p-8 relative z-20 shadow-[0_0_100px_rgba(168,85,247,0.1)]">
        ...
        {/* STATUS BAR */}
        <div className="mb-8 bg-white/5 border border-white/5 p-2 flex justify-between items-center">
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <button 
            disabled={loading}
            className="w-full py-5 bg-[#a855f7] text-white font-black uppercase text-[10px] tracking-[0.3em] hover:bg-[#b975ff] transition-all flex items-center justify-center gap-2 group shadow-lg shadow-[#a855f7]/20 active:scale-95"
          >
